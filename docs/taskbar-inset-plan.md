...
## Phase 2 — runtime robustness (optional, ships independently)
...
Fix: cache a lightweight taskbar signature `(edge, reservesSpace)` from the last
build. In `OnStripPosChanged`, recompute it; if it changed → `ScheduleRebuild()`,
else reposition as today. This keeps reposition cheap for ordinary appbar moves
while picking up taskbar relocations and auto-hide toggles.
